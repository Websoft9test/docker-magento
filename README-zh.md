...
安装本项目，确保符合如下的条件（[详细参考](https://devdocs.magento.com/cloud/docker/docker-development.html#prerequisites)）：
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r magento
...
```
sudo wget -N https://raw.githubusercontent.com/Websoft9/StackHub/main/docker-installer.sh; sudo bash docker-installer.sh -r magento -p
```
2.复制package包后安装

将压缩的package包复制到想要安装的服务器或虚拟机，登录 Linux，运行下面的**自动化命令**即可启动安装并显示安装结果。
```
sudo bash install-magento
```
...
```
git clone --depth=1 https://github.com/Websoft9/docker-magento
cd docker-magento
